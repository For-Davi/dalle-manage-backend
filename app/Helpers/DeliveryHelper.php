...
class DeliveryHelper
{
    ...
    {
        $allAreZero = collect($partialDeliveredData)->every(fn ($item) => (int) ($item['quantityDelivered'] ?? 0) === 0
        );
    ...
    {
        $allAreDelivered = collect($partialDeliveredData)->every(fn ($item) => (int) ($item['quantityDelivered'] === $item['quantitySaled'])
        );
